...
app = Flask(__name__)
df = pd.read_csv(r'D:\freshForecast-main\freshForecast-main\flask_app\dataset.csv')
...
model = load_model('lstm_model.h5', custom_objects={'LSTM': custom_lstm, 'mse': MeanSquaredError()})
...
data = pd.read_csv(r'D:\freshForecast-main\freshForecast-main\flask_app\dataset.csv')
...
def predict():
    if request.method == "POST":
        ...
        assert len(commodity_data) + 3 == EXPECTED_FEATURE_COUNT, f"Expected {EXPECTED_FEATURE_COUNT} features, got {len(commodity_data) + 3}"
